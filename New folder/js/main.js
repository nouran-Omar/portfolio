/* ================================================
   NOURAN OMAR PORTFOLIO — COMPLETE JAVASCRIPT
   ================================================ */
...
const dictionary = {
  en: {
    ...
    "stat-gpa":"GPA","stat-grad":"Grad Project","stat-proj":"Projects",
  ...
  ar: {
    ...
    "stat-gpa":"المعدل","stat-grad":"مشروع التخرج","stat-proj":"مشروع",
    "btn-hire":"وظفني","btn-portfolio":"أعمالي","btn-cv":"السيرة الذاتية",
    // About
    "about-tag":"من أنا","about-head-1":"نبذة","about-head-2":"عني",
    "about-text-p":"مهندسة برمجيات شغوفة، من المتوقع تخرجها عام 2026 بمعدل تراكمي 3.67 من EELU. متخصصة في بناء تطبيقات قوية وقابلة للتطوير، وتحويل التصاميم المعقدة إلى واجهات مستخدم تفاعلية وسلسة بكود نظيف ومنظم.",
...
const experienceData = [
  { id:"exp1", side:"left",  icon:"fa-solid fa-code" },
  { id:"exp2", side:"right", icon:"fa-solid fa-briefcase" },
];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyLang(lang) {
    ...
    // Re-translate dynamic rendered content
    ...
    if(ftxt) {
      ftxt.textContent = lang === "en" ? "Eng. Nouran" : "م. نوران";
    }
  ...
  function buildLogo(lang) {
    ...
    if(lang === "en") {
      // Letters wave up/down
      const letters = "NO";
      mono.innerHTML = letters.split("").map(c =>
        `<span class="lc">${c}</span>`
      ).join("");
      if(fullName) fullName.textContent = "Eng. Nouran";
      if(roleTag)  roleTag.textContent  = "Frontend Dev";
    } else {
      // Arabic with tashkeel — full styled text
      mono.innerHTML = `<span class="lc" style="font-family:'Cairo',sans-serif;font-size:1.3rem;">الْمُهَنْدِسَة نُورَانْ</span>`;
  ...
  function restartTypewriter(lang) {
    ...
    setTimeout(() => startTypewriter(lang), 300);
  }
    ...
    function tick() {
      ...
      if(state.twDel) {
        ...
        if(state.twChar === 0) {
          ...
          state.twTimer = setInterval(tick, 90);
        }
      } else {
        el.textContent = cur.substring(0, state.twChar + 1);
        state.twChar++;
        if(state.twChar === cur.length) {
          state.twDel = true;
          clearInterval(state.twTimer);
          setTimeout(() => { state.twTimer = setInterval(tick, 55); }, 1800);
        }
      }
    }
    state.twTimer = setInterval(tick, 90);
  ...
  function renderSkills() {
    ...
    if(!c) return;
    c.innerHTML = skillCategories.map(item => `
  ...
  function renderExperience() {
    ...
    tl.innerHTML = experienceData.map((item, i) => `
      <div class="timeline-item timeline-${item.side}"
           data-aos="fade-${item.side === "left" ? "right" : "left"}"
           data-aos-delay="${i * 150}">
        <div class="timeline-content">
          <div class="tl-icon"><i class="${item.icon}"></i></div>
  ...
  function renderProjects() {
    ...
    list.innerHTML = coreProjects.map((p, i) => `
      <div class="project-entry" data-aos="fade-up" data-aos-delay="${Math.min(i * 70, 300)}">
        ...
        <div class="project-details">
          ...
          <h3 class="proj-title" data-i18n="proj-${p.id}-title"></h3>
          <p class="proj-desc"  data-i18n="proj-${p.id}-desc"></p>
          ...
          <div class="proj-links">
            <a href="${p.link}" target="_blank" rel="noopener noreferrer" class="lnk-code">
              ...
              <span data-i18n="proj-view-code">View Code</span>
            </a>
            ${p.demoLink ? `
            <a href="${p.demoLink}" target="_blank" rel="noopener noreferrer" class="lnk-demo">
              <i class="fas fa-external-link-alt"></i>
              <span data-i18n="proj-live-demo">Live Demo</span>
            </a>` : ""}
          </div>
        </div>
      </div>
  ...
  function renderCertificates() {
    ...
    c.innerHTML = certificatesData.map((cert, i) => `
      <div class="certificate-card" data-aos="zoom-in-up" data-aos-delay="${i * 120}">
        ...
        <a href="${cert.file}" target="_blank" rel="noopener noreferrer" class="btn btn-outline">
          ...
          <span data-i18n="btn-download">Download PDF</span>
        </a>
      </div>
    `).join("");
  }
    ...
    form.addEventListener("submit", e => {
      ...
      if(typeof emailjs === "undefined") {
        setTimeout(() => Swal.fire({
          ...
          text: "EmailJS not loaded. Please configure your keys.",
          background: bgCard, confirmButtonColor: accent,
        }), 500);
        return;
      }
  ...
  function initTimeline() {
    ...
    function update() {
      ...
      const bot = Math.max(0,  r.bottom - vh);
      const vis = r.height - top - bot;
      let pct = top > 0 ? (top / (r.height - vis)) * 100 : 0;
      pct = Math.max(0, Math.min(100, pct));
      tl.style.setProperty("--timeline-progress", `${pct}%`);
    }
  ...
  function initParticles() {
    ...
    particlesJS("particles-js", {
      particles: {
        ...
        color:{ value:"#FDCEDF" },
        shape:{ type:"circle" },
        opacity:{ value:.22, random:true, anim:{ enable:true, speed:.6, opacity_min:.05 } },
        size:{ value:2.5, random:true },
        line_linked:{ enable:true, distance:150, color:"#867070", opacity:.2, width:1 },
  ...
  renderProjects();
  renderCertificates();
  initLang();          // must run after renders so dynamic content gets translated
  initContactForm();
  initScrollSpy();
  initTimeline();
  initAnimations();
  ...
  setTimeout(() => startTypewriter(state.lang), 1300);
});